...
This simulation was created to answer one question: is it possible for several hospitals to check a model's calibration without combining their patient-level data in one place?
...
I compared the federated measurements with the same calculations performed on a combined patient-level dataset. The results matched, as expected, because all hospitals used the same predetermined probability ranges.
